...
signup : { email : String, password : String } -> Cmd Msg
signup payload =
    Http.request
        { method = "POST"
        , url = "/api/signup"
...
body model =
    div [ class "main" ]
        [ div [ class "form" ]
            ...
            , button [ class "form", onClick Signup ] [ text "Create account" ]
            , a [ href "/app/login" ] [ text "Already have an account? Login" ]
            ]
        ]
        ...
        GotAuth response ->
            case response of
                RemoteData.Success auth ->
                    ( { model
                        | message = Just (Message.Success "You're in, let's go!")
                      }
